...
public interface StudyDiaryRepository extends JpaRepository<StudyDiary, Long> {
    ...
    @Query("SELECT s FROM StudyDiary s LEFT JOIN FETCH s.user WHERE s.isCreated = true AND s.createdAt BETWEEN :startOfWeek AND :endOfWeek ORDER BY s.likeCount DESC LIMIT 50")
